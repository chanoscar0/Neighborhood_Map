...
var ViewModel = function() {
  ...
  ViewModel.prototype.initMap = function() {
    ...
    function populateInfoWindow(marker, infowindow) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        data: 'll=' + lat + ',' + lng + '&limit=1' + '&client_id=' + clientID + '&client_secret=' + clientSecret + '&v=20170801' + '&m=foursquare',
        async: true,
        ...
        success: function(data) {
          ...
          contentString = locationName + ' is located at ' + locationFormattedAddress[0] + ' ' + locationFormattedAddress[1] + ' ' + locationFormattedAddress[2];
          ...
          $.ajax({
            ...
            dataType: 'json',
            data: '&client_id=' + clientID + '&client_secret=' + clientSecret + '&v=20170801' + '&m=foursquare',
            async: true,
            ...
            success: function(data) {
              if (data.response.photos.items.length == 0) {
                ...
                imageString = "<div style = float:left;> <img src = " + imageString + "></div>";
                contentString = imageString + contentString;
                if (infowindow.marker != marker) {
                  infowindow.marker = marker;
                  infowindow.setContent(contentString);
  ...
  self.highLightMenuIcon = function(clickedMarker) {
    for (var i =0; i < locations.length; i ++){
      if (clickedMarker.title == locations[i].title){
